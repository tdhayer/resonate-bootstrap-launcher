...
# It fetches the private Proxmox installer script through the GitHub API
# using an authenticated token, then executes it locally.

log() {
  printf '[resonate-bootstrap] %s\n' "$*"
}
...
RESONATE_PRIVATE_SCRIPT_PATH="${RESONATE_PRIVATE_SCRIPT_PATH:-scripts/proxmox-bootstrap.sh}"
...
TMP_SCRIPT="$(mktemp -t resonate-private-installer.XXXXXX.sh)"

log 'Fetching private installer from GitHub API...'
curl -fsSL \
...
# Ensure the private installer has repo access when it clones into the LXC.
...
log 'Launching private Proxmox installer...'
bash "${TMP_SCRIPT}"

# Best-effort cleanup of token from the current shell environment.
unset GH_TOKEN
